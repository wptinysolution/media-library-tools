...
export default function ExifStripperPage() {
    ...
    const toggleSelectAll = () => {
        if (selectedIds.size === images.length) {
            ...
            setSelectedIds(new Set(images.filter(img => img.has_exif && !img.stripped).map(img => img.attachment_id)));
        }
    };
                                ...
                                >
                                    <div className="flex items-center gap-4 p-4">
                                        ...
                                        {image.has_exif && !image.stripped && (
                                            <div
                                                className="shrink-0"
                                                onClick={() => toggleSelect(image.attachment_id)}
                                            >
                                                <input
                                                    type="checkbox"
                                                    className="w-4 h-4 rounded border-gray-300 text-blue-600 cursor-pointer"
